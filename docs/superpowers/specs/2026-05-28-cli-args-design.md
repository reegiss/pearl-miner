...
```
pearl-miner --wallet pearl1q... --pool stratum+tcp://pool.pearl.io:3333
```
...
pub struct Args {
    /// Endereço Taproot da carteira Pearl (bech32m, prefixo pearl1)
    #[arg(long)]
    pub wallet: String,

    /// URL da pool de mineração (ex: stratum+tcp://host:3333)
    #[arg(long)]
    pub pool: String,
}
...
impl Args {
    ...
    /// Validate the pool URL (must contain "://").
...
2. Check `variant == bech32::Variant::Bech32m` — Pearl uses bech32m, not bech32.
3. Check `hrp == "pearl"` — rejects addresses from other networks.
4. Convert the 5-bit data to 8-bit bytes via `bech32::convert_bits`.
...
- Returns `Err` if the URL does not contain `"://"`.
- No connection attempt — actual protocol handling belongs to the future P2P module.
...
| Wrong network prefix | `error: endereço da carteira inválido: HRP deve ser 'pearl', encontrado '<hrp>'` |
| Not bech32m variant | `error: endereço da carteira inválido: deve ser bech32m (não bech32)` |
| Bad checksum | `error: endereço da carteira inválido: checksum bech32m incorreto` |
| Pool missing `://` | `error: URL da pool inválida: deve conter '://' (ex: stratum+tcp://host:3333)` |
...
Options:
      --wallet <WALLET>  Endereço Taproot da carteira Pearl (bech32m, prefixo pearl1)
      --pool <POOL>      URL da pool de mineração (ex: stratum+tcp://host:3333)
  -h, --help             Print help
...
- `wallet_bytes()` with a valid bech32m address returns the correct bytes.
- `wallet_bytes()` with wrong HRP (`bc1q...`) returns an error containing `'pearl'`.
...
- `validate_pool()` accepts `stratum+tcp://host:3333`.
- `validate_pool()` rejects `"host:3333"` (no `://`).
