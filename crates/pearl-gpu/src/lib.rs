...
static BLAKE3_PTX_SM75: &str = include_str!(env!("BLAKE3_PTX_SM75"));

#[cfg(has_sm86_ptx)]
static PTX_SM86: &str = include_str!(env!("MATMUL_PTX_SM86"));

#[cfg(has_blake3_sm86_ptx)]
static BLAKE3_PTX_SM86: &str = include_str!(env!("BLAKE3_PTX_SM86"));

#[cfg(has_sm89_ptx)]
static PTX_SM89: &str = include_str!(env!("MATMUL_PTX_SM89"));

#[cfg(has_blake3_sm89_ptx)]
static BLAKE3_PTX_SM89: &str = include_str!(env!("BLAKE3_PTX_SM89"));
...
fn best_blake3_ptx(sm: u32) -> &'static str {
    #[cfg(has_blake3_sm89_ptx)]
    if sm >= 89 { return BLAKE3_PTX_SM89; }
    #[cfg(has_blake3_sm86_ptx)]
    if sm >= 86 { return BLAKE3_PTX_SM86; }
    let _ = sm;
    BLAKE3_PTX_SM75
}
...
impl GpuMiner {
    pub fn new(device_id: usize, params: &MiningParams) -> Result<Self, GpuError> {
        ...
        let blake3_module = ctx.load_module(Ptx::from_src(best_blake3_ptx(sm)))?;
        ...
        let func_solve  = blake3_module.load_function("solve_pool_challenge_real")?;
        let use_wmma    = sm >= 72;
        ...
        Ok(Self {
            ctx, stream, module, blake3_module,
            func_dp4a, func_wmma, func_blake3, func_gen_a, func_solve, use_wmma,
    ...
    /// Run matmul + GPU-side BLAKE3 difficulty check.
    /// Returns found PoUW blocks and stage timings [kernel_us, dtoh_us, 0].
    pub fn mine(
        &self,
        params:   &MiningParams,
        job_seed: u64,
        ...
        // Stage 0: generate A' = A + EL·ER on GPU (EL stays as zeros; noise disabled
        // for now — enables B' L2 cache to stay warm for the WMMA kernel).
        {
            let bx: u32 = 32;
            let by: u32 = 16;
    ...
    pub fn solve_pool_challenge(
        ...
        // Use the isolated blake3_module to avoid register pressure on WMMA kernels
        ...
        // 256 blocks × 1024 threads = 262 144 nonces per launch
        ...
        loop {
            ...
            let mut b = self.stream.launch_builder(&self.func_solve);
            b.arg(&d_seed);
            b.arg(&base_nonce);
            b.arg(&difficulty);
            b.arg(&mut d_nonce);
